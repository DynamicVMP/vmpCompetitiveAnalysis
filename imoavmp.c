...
int main (int argc, char *argv[]) {
    ...
    else
    {
        ...
        int v_size = get_v_size_per_t(S,t,s_size);
        float **V = load_v_per_t(S,s_size,v_size,t);
        /*printf("\nH\n\n");
        ...
        create_structures(&P,&Q,&utilization_P,&utilization_Q,&weighted_sums_P,&weighted_sums_Q,&objectives_functions_values_aux,NUMBER_OF_INDIVIDUALS,v_size,h_size);
        ...
        while (generation < NUMBER_OF_GENERATIONS)
        {
            ...
            /* Additional task: calculate the cost of each objective function for each solution */
            weighted_sums_Q = load_weighted_sums(objectives_functions_values_aux, weighted_sums_Q, Q, utilization_Q, H,
                                                 V, NUMBER_OF_INDIVIDUALS, h_size, v_size);
            ...
            utilization_P = load_utilization(utilization_P,P, H, V, NUMBER_OF_INDIVIDUALS, h_size, v_size);
            weighted_sums_P = load_weighted_sums(objectives_functions_values_aux, weighted_sums_P, P, utilization_P, H,
                                                 V, NUMBER_OF_INDIVIDUALS, h_size, v_size);
